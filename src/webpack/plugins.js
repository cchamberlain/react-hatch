...
const { CommonsChunkPlugin, UglifyJsPlugin, OccurenceOrderPlugin, DedupePlugin } = optimize
...
export const getPlugins = name => {
  ...
  if(/^win/.test(process.platform)) plugins.push(new IgnorePlugin(/dtrace-provider/i))
  //if(IS_DEV) plugins.push(new optimize.DedupePlugin())
